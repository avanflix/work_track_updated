...
export async function PATCH(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Admin-requested transfers need Super Admin approval; employee-requested transfers need an Admin (or Super Admin).
  ...
  if (!can(role, requiredPermission)) {
    return NextResponse.json({ error: "You do not have permission to approve this transfer" }, { status: 403 });
  }

  const approver = { id: session.user.id as string, name: session.user.name ?? "Admin" };
  const updated = await transferService.decide(id, "APPROVED", approver);

  await notificationService.create({
    recipient: request.requestedBy.toString(),
    title: "Transfer approved",
    message: `${approver.name} approved your transfer request for "${request.task?.title ?? "the task"}"`,
    type: "GENERAL",
    relatedTask: request.task?._id?.toString() ?? request.task?.toString(),
  });
  await notificationService.create({
    recipient: request.transferTo.toString(),
    title: "Task transferred to you",
    message: `"${request.task?.title ?? "A task"}" has been transferred to you`,
    type: "TASK_ASSIGNED",
    relatedTask: request.task?._id?.toString() ?? request.task?.toString(),
  });
